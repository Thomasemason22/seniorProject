...
function DailyReportPanel({ data, kpis }) {
  const scanRate = kpis.total_volume ? (kpis.total_scanned / kpis.total_volume) * 100 : 0;
  ...
  return (
    <section className="report-panel" id="report">
      ...
      <div className="report-grid">
        ...
        <div><span>Scanned volume</span><strong>{Math.round(kpis.total_scanned || 0).toLocaleString()}</strong></div>
        <div><span>Scan rate</span><strong>{scanRate.toFixed(1)}%</strong></div>
